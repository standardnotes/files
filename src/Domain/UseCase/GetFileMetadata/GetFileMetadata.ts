...
export class GetFileMetadata implements UseCaseInterface {
  ...
  async execute(dto: GetFileMetadataDTO): Promise<GetFileMetadataResponse> {
    try {
      const size = await this.fileDownloader.getFileSize(`${dto.userUuid}/${dto.resource}`)
      ...
      this.logger.error(`Could not get file metadata for resource: ${dto.userUuid}/${dto.resource}`)
      return {
        success: false,
