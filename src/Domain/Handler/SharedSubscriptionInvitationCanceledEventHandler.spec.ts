...
import { SharedSubscriptionInvitationCanceledEvent, SharedSubscriptionInvitationCanceledEventPayload, DomainEventPublisherInterface, FileRemovedEvent } from '@standardnotes/domain-events'
...
describe('SharedSubscriptionInvitationCanceledEventHandler', () => {
  ...
  const createHandler = () => new SharedSubscriptionInvitationCanceledEventHandler(markFilesToBeRemoved, domainEventPublisher, domainEventFactory)
  ...
  beforeEach(() => {
    ...
    markFilesToBeRemoved.execute = jest.fn().mockReturnValue({
      success: true,
      filesRemoved: [
        {} as jest.Mocked<RemovedFileDescription>,
      ],
    })
